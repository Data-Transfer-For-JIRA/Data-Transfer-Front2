...
export default function SerchInput<T>({ handleSearchResult }: SerchProjectInputType<T>) {
  const [searhKeyword, setSearchKeyword] = useState<string>("");

  const handleSerchBtn = () => {
    //핸들러 함수도 props로 전달받으면 재활용성이 더높아지나?
    const serchApiResult = GetAxiosSearchJiraList(searhKeyword);
    serchApiResult.then((data) => {
      if (data !== undefined) {
  ...
  const handleChangeKeyword = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    console.log(searhKeyword);
    setSearchKeyword(event.target.value);
  }

  const handleKeyDown = async (event: React.KeyboardEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    if (event.key === 'Enter') {
      const serchApiResult = await GetAxiosSearchJiraList(searhKeyword);
      if (serchApiResult !== undefined) { handleSearchResult(serchApiResult); }
    }
  }
  return (
    <Box sx={{ border: '2px solid grey' }} height={70}>
      <FormControl>
        ...
        />
        <IconButton type="button" sx={{ p: '10px' }} aria-label="search" onClick={handleSerchBtn}>
          <SearchIcon />
        </IconButton>
      </FormControl>
    </Box>
